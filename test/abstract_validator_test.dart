...
void main() {
  ...
  setUp(() async {
    contact =  Contact();
    myValidator =  ContactValidator(contact);
  });
  ...
  test('validate all, when 4 errors on 3 properites, has correct validation results and messages', () {
    ...
    myValidator.ruleFor('password', () => contact.password)
      ..length(16, 16)
      ..notEqual(() => 'password');
  ...
  test('all error test, when 4 errors, shows correct error text', () {
    ...
    expect(allErrors,
        'name must be specified. phone is not valid. password must be between 16 and 16. password must not equal .');
  });

  test('when using must validator, condition passes as true, errors are empty', () {
    nameMustNotBeFred(String value) {
      return value != 'fred';
    }
