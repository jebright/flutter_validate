...
void main() {
  ...
  test('yyyy-mm-dd date, isValid called with a custom date format (ISO 8601), is True', () {
    expect(DateValidator.isValidDate('2000-02-14', 'yyyy-MM-dd'), true);
  });

  test('yyyy-mm-dd date, isValid called with a custom date format (ISO 8601) but a bad date, is false', () {
    expect(DateValidator.isValidDate('2000-13-01', 'yyyy-MM-dd'), false); //No 13th month!
  });

  //
  test('7/10/1996 5:08 PM date and time, isValid called with a custom date format (d/M/yyyy h:m a), is true', () {
    expect(DateValidator.isValidDate('7/10/1996 5:08 PM', 'd/M/yyyy h:m a'), true);
  });
  ...
  test('Great Britain date (dd/mm/yy), isValid called, is True', () {
    ...
    initializeDateFormatting('en_GB', null).then((x) => expect(DateValidator.isValidDate('31/7/1973'), true));
  });
}
